// src/app/admin/services/edit/[id]/page.tsx (FIXED VERSION)
"use client";
...
export default function EditServicePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log('Fetching service with publicId:', params.id);
        ...
        if (!serviceRes.ok) {
          ...
          throw new Error(errorData.error || "Gagal memuat jasa");
        }
        
        const serviceData = await serviceRes.json();
        console.log('Service data received:', serviceData);

        // Fetch categories with SERVICE type filter
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if ((service?.images.length || 0) - imagesToDelete.length + images.length + newFiles.length > 5) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log('Sending PUT request to:', `/api/services/${params.id}`);
      ...
      router.push(`/admin/services/view/${params.id}`);
    } catch (error: any) {
      console.error("Error updating service:", error);
      toast.error(error.message || "Gagal mengupdate jasa");
    } finally {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 text-center">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-4">
          Jasa dengan ID "{params.id}" tidak ditemukan atau telah dihapus.
        </p>
        <Link
          href="/admin/services"
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          <Link
            href={`/admin/services/view/${params.id}`}
            className="mr-4 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800"
          >
            <ArrowLeft size={20} />
          </Link>
          <div>
            <h1 className="text-2xl font-bold">Edit Jasa</h1>
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-green-600 dark:text-green-400">
              <span className="font-medium">Service ID:</span> {service.publicId}
            </p>
            <p className="text-sm text-green-600 dark:text-green-400">
              <span className="font-medium">Kategori:</span> {service.category?.name}
            </p>
          </div>
              ...
              <div>
                ...
                {categories.length === 0 && (
                  <p className="mt-1 text-sm text-amber-600">
                    ...
                    <Link href="/admin/categories/services/create" className="text-primary-600 hover:text-primary-700 ml-1">
                      Buat kategori baru
                    </Link>
                  </p>
                )}
                ...
                <div className="flex items-center justify-center w-full">
                  <label
                    ...
                    className={`flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-md dark:border-gray-700 cursor-pointer bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 ${
                      (service.images.length - imagesToDelete.length + images.length) >= 5 ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                  >
                    ...
                    <input
                      ...
                      disabled={(service.images.length - imagesToDelete.length + images.length) >= 5}
                    />
                  </label>
                </div>
                
                {(service.images.length - imagesToDelete.length + images.length) >= 5 && (
