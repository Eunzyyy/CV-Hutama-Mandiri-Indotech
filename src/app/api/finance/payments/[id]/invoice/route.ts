// src/app/api/finance/payments/[id]/invoice/route.ts - SUPER CLEAN VERSION
...
export async function GET(
  ...
  try {
    ...
    doc.text('CV HUTAMA MANDIRI INDOTECH', 45, 20);
    ...
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    doc.text(`FAKTUR No. : ${payment.order.orderNumber}/HMI/IX/${currentDate.getFullYear()}`, 10, 62);
    ...
    const customerName = payment.order.user.name.toUpperCase();
    ...
    // Table bottom line
    const tableEndY = currentRowY + (payment.order.orderItems.length * rowHeight) + rowHeight;
    ...
    doc.text(`Status: ${payment.status === 'PAID' ? 'LUNAS' : 'BELUM LUNAS'}`, 10, infoStartY + 7);
