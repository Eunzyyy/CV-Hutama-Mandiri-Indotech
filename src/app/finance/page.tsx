// src/app/finance/payments/page.tsx - FULL VERSION WITH DOWNLOAD INVOICE
"use client";
...
export default function FinancePaymentsPage() {
  ...
  const downloadInvoice = async (paymentId: number, orderNumber: string) => {
    try {
      setDownloadingInvoice(paymentId);
      const response = await fetch(`/api/finance/payments/${paymentId}/invoice`);
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `Faktur-${orderNumber}.pdf`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);
        toast.success('Faktur berhasil didownload');
      } else {
        const error = await response.json();
        throw new Error(error.error || 'Gagal download faktur');
      }
    } catch (error: any) {
      console.error('Error downloading invoice:', error);
      toast.error(error.message || 'Gagal download faktur');
    } finally {
                    ...
                    <tr key={payment.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <div className="flex space-x-2">
                          ...
                          <button
                            onClick={() => downloadInvoice(payment.id, payment.order.orderNumber)}
